...
TEST(Reader_Common, construct_failure)
{
    ...
    EXPECT_THROW(TextStreamReader::open(is, Charset::CP_850), std::ios::failure);
}
...
TEST(Reader_Common, read_n_failure)
{
    ...
    std::istream is(&streambuf);
    is.exceptions(std::ios_base::failbit | std::ios_base::badbit);
    std::unique_ptr<TextStreamReader> reader = TextStreamReader::open(is, Charset::CP_850);
...
TEST(Reader_Common, unknown_encoding)
{
    std::string input = "Hallo Welt!";
    std::istringstream iss(input);
    EXPECT_THROW(TextStreamReader::open(iss, static_cast<Charset>(255)), std::invalid_argument);
}
