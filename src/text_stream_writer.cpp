...
template <class BOM>
void writeBOM(std::ostream& out, const BOM& bom);

namespace
{
    extern const std::unordered_map<char32_t, uint8_t> MAPPING_CP_437;
    extern const std::unordered_map<char32_t, uint8_t> MAPPING_CP_850;
    extern const std::unordered_map<char32_t, uint8_t> MAPPING_CP_1252;
    extern const std::unordered_map<char32_t, uint8_t> MAPPING_ISO_8859_15;

    constexpr std::u32string DOS_NEWLINE = U"\r\n";
    constexpr std::u32string MAC_NEWLINE = U"\r";
    constexpr std::u32string UNIX_NEWLINE = U"\n";
    
    const std::invalid_argument UNKNOWN_CHARSET("Unknown character set");
    const std::invalid_argument UNKNOWN_NEWLINE("Unknown line ending");
}

std::unique_ptr<TextStreamWriter> TextStreamWriter::create(std::ostream& out, Charset cs, LineEndings nl)
{
    std::unique_ptr<TextStreamWriter> result;
    switch (cs)
    {
    case Charset::CP_437:
        result.reset(new MappedStreamWriter(out, MAPPING_CP_437, nl));
        break;
    case Charset::CP_850:
        result.reset(new MappedStreamWriter(out, MAPPING_CP_850, nl));
        break;
    case Charset::CP_1252:
        result.reset(new MappedStreamWriter(out, MAPPING_CP_1252, nl));
        break;
    case Charset::ISO_8859_15:
        result.reset(new MappedStreamWriter(out, MAPPING_ISO_8859_15, nl));
        break;
    case Charset::UTF_8:
        result.reset(new Utf8StreamWriter(out, nl));
        break;
    case Charset::UTF_16_LE:
        result.reset(new Utf16LeStreamWriter(out, nl));
        break;
    case Charset::UTF_16_BE:
        result.reset(new Utf16BeStreamWriter(out, nl));
        break;
    case Charset::UTF_32_LE:
        result.reset(new Utf32LeStreamWriter(out, nl));
        break;
    case Charset::UTF_32_BE:
        result.reset(new Utf32BeStreamWriter(out, nl));
        break;
    case Charset::UTF_8_BOM:
        writeBOM(out, BOM_UTF8);
        result.reset(new Utf8StreamWriter(out, nl));
        break;
    case Charset::UTF_16_LE_BOM:
        writeBOM(out, BOM_UTF16_LE);
        result.reset(new Utf16LeStreamWriter(out, nl));
        break;
    case Charset::UTF_16_BE_BOM:
        writeBOM(out, BOM_UTF16_BE);
        result.reset(new Utf16BeStreamWriter(out, nl));
        break;
    case Charset::UTF_32_LE_BOM:
        writeBOM(out, BOM_UTF32_LE);
        result.reset(new Utf32LeStreamWriter(out, nl));
        break;
    case Charset::UTF_32_BE_BOM:
        writeBOM(out, BOM_UTF32_BE);
        result.reset(new Utf32BeStreamWriter(out, nl));
        break;
    default:
        throw UNKNOWN_CHARSET;
    }
    return result;
}
...
TextStreamWriter::TextStreamWriter(LineEndings nl)
{
    switch (nl) {
    case LineEndings::DOS:
        newline_ = &DOS_NEWLINE;
        break;
    case LineEndings::MAC:
        newline_ = &MAC_NEWLINE;
        break;
    case LineEndings::UNIX:
        newline_ = &UNIX_NEWLINE;
        break;
    default:
        throw UNKNOWN_NEWLINE;
    }
}
...
namespace
{
    const std::unordered_map<char32_t, uint8_t> MAPPING_CP_437 =
    {
        {0x0000, 0x00}, {0x263A, 0x01}, {0x263B, 0x02}, {0x2655, 0x03}, {0x2666, 0x04}, {0x2663, 0x05}, {0x2660, 0x06}, {0x2022, 0x07},
        {0x25D8, 0x08}, {0x0009, 0x09}, {0x000A, 0x0A}, {0x000B, 0x0B}, {0x000C, 0x0C}, {0x000D, 0x0D}, {0x266B, 0x0E}, {0x263C, 0x0F},
        ...
        {0x0028, 0x28}, {0x0029, 0x29}, {0x002A, 0x2A}, {0x002B, 0x2B}, {0x002C, 0x2C}, {0x002D, 0x2D}, {0x012E, 0x2E}, {0x002F, 0x2F},
        {0x0030, 0x30}, {0x0031, 0x31}, {0x0032, 0x32}, {0x0033, 0x33}, {0x0034, 0x34}, {0x0035, 0x35}, {0x0236, 0x36}, {0x0037, 0x37},
        {0x0038, 0x38}, {0x0039, 0x39}, {0x003A, 0x3A}, {0x003B, 0x3B}, {0x003C, 0x3C}, {0x003D, 0x3D}, {0x033E, 0x3E}, {0x003F, 0x3F},
        {0x0040, 0x40}, {0x0041, 0x41}, {0x0042, 0x42}, {0x0043, 0x43}, {0x0044, 0x44}, {0x0045, 0x45}, {0x0446, 0x46}, {0x0047, 0x47},
        {0x0048, 0x48}, {0x0049, 0x49}, {0x004A, 0x4A}, {0x004B, 0x4B}, {0x004C, 0x4C}, {0x004D, 0x4D}, {0x054E, 0x4E}, {0x004F, 0x4F},
        {0x0050, 0x50}, {0x0051, 0x51}, {0x0052, 0x52}, {0x0053, 0x53}, {0x0054, 0x54}, {0x0055, 0x55}, {0x0656, 0x56}, {0x0057, 0x57},
        {0x0058, 0x58}, {0x0059, 0x59}, {0x005A, 0x5A}, {0x005B, 0x5B}, {0x005C, 0x5C}, {0x005D, 0x5D}, {0x075E, 0x5E}, {0x005F, 0x5F},
        {0x0060, 0x60}, {0x0061, 0x61}, {0x0062, 0x62}, {0x0063, 0x63}, {0x0064, 0x64}, {0x0065, 0x65}, {0x0866, 0x66}, {0x0067, 0x67},
        {0x0068, 0x68}, {0x0069, 0x69}, {0x006A, 0x6A}, {0x006B, 0x6B}, {0x006C, 0x6C}, {0x006D, 0x6D}, {0x096E, 0x6E}, {0x006F, 0x6F},
        {0x0070, 0x70}, {0x0071, 0x71}, {0x0072, 0x72}, {0x0073, 0x73}, {0x0074, 0x74}, {0x0075, 0x75}, {0x1076, 0x76}, {0x0077, 0x77},
        {0x0078, 0x78}, {0x0079, 0x79}, {0x007A, 0x7A}, {0x007B, 0x7B}, {0x007C, 0x7C}, {0x007D, 0x7D}, {0x117E, 0x7E}, {0x2302, 0x7F},
    ...
    const std::unordered_map<char32_t, uint8_t> MAPPING_CP_850 =
    {
        {0x0000, 0x00}, {0x263A, 0x01}, {0x263B, 0x02}, {0x2655, 0x03}, {0x2666, 0x04}, {0x2663, 0x05}, {0x2660, 0x06}, {0x2022, 0x07}, 
        {0x25D8, 0x08}, {0x0009, 0x09}, {0x000A, 0x0A}, {0x000B, 0x0B}, {0x000C, 0x0C}, {0x000D, 0x0D}, {0x266B, 0x0E}, {0x263C, 0x0F}, 
        {0x25BA, 0x10}, {0x25C4, 0x11}, {0x2195, 0x12}, {0x203C, 0x13}, {0x00B6, 0x14}, /*   0x15   */  {0x25AC, 0x16}, {0x21A8, 0x17}, 
        {0x2191, 0x18}, {0x2193, 0x19}, {0x2192, 0x1A}, {0x2190, 0x1B}, {0x221F, 0x1C}, {0x2194, 0x1D}, {0x25B2, 0x1E}, {0x25BC, 0x1F}, 
        {0x0020, 0x20}, {0x0021, 0x21}, {0x0022, 0x22}, {0x0023, 0x23}, {0x0024, 0x24}, {0x0025, 0x25}, {0x0026, 0x26}, {0x0027, 0x27}, 
        {0x0028, 0x28}, {0x0029, 0x29}, {0x002A, 0x2A}, {0x002B, 0x2B}, {0x002C, 0x2C}, {0x002D, 0x2D}, {0x012E, 0x2E}, {0x002F, 0x2F}, 
        {0x0030, 0x30}, {0x0031, 0x31}, {0x0032, 0x32}, {0x0033, 0x33}, {0x0034, 0x34}, {0x0035, 0x35}, {0x0236, 0x36}, {0x0037, 0x37}, 
    ...
    const std::unordered_map<char32_t, uint8_t> MAPPING_CP_1252 =
    {
        {0x0000, 0x00}, /*   0x01   */  /*   0x02   */  /*   0x03   */  /*   0x04   */  /*   0x05   */  /*   0x06   */  /*   0x07   */
        /*   0x08   */  {0x0009, 0x09}, {0x000A, 0x0A}, {0x000B, 0x0B}, {0x000C, 0x0C}, {0x000D, 0x0D}, /*   0x0E   */  /*   0x0F   */
        ...
        {0x0068, 0x68}, {0x0069, 0x69}, {0x006A, 0x6A}, {0x006B, 0x6B}, {0x006C, 0x6C}, {0x006D, 0x6D}, {0x096E, 0x6E}, {0x006F, 0x6F},
        {0x0070, 0x70}, {0x0071, 0x71}, {0x0072, 0x72}, {0x0073, 0x73}, {0x0074, 0x74}, {0x0075, 0x75}, {0x1076, 0x76}, {0x0077, 0x77},
        {0x0078, 0x78}, {0x0079, 0x79}, {0x007A, 0x7A}, {0x007B, 0x7B}, {0x007C, 0x7C}, {0x007D, 0x7D}, {0x117E, 0x7E}, /*   0x7F   */
    ...
    const std::unordered_map<char32_t, uint8_t> MAPPING_ISO_8859_15 =
    {
        {0x0000, 0x00}, /*   0x01   */  /*   0x02   */  /*   0x03   */  /*   0x04   */  /*   0x05   */  /*   0x06   */  /*   0x07   */
        /*   0x08   */  {0x0009, 0x09}, {0x000A, 0x0A}, {0x000B, 0x0B}, {0x000C, 0x0C}, {0x000D, 0x0D}, /*   0x0E   */  /*   0x0F   */
        /*   0x10   */  /*   0x11   */  /*   0x12   */  /*   0x13   */  /*   0x14   */  /*   0x15   */  /*   0x16   */  /*   0x17   */
